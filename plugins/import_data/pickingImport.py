...
class TRaILpicking(BaseImporter):
    ...
    def import_datafile(self, fn, rec, **kwargs):
        ...
        data = data[
            (data[self.picking_specs["Metadata"]["Researcher"]].notnull())
            & (data["Sample"] != "EXAMPLE")
        ]
        ...
        for d in range(len(data)):
            ...
            material = self.picking_specs["mineral_key"][
                data.iloc[d][self.picking_specs["Metadata"]["Mineral"]]
            ]
            ...
            if shard != "Y" and shard != "y":
                length1 = data.iloc[d][
                    self.picking_specs["Metadata"]["Dimensions"]["Length 1"]
                ]
                width1 = data.iloc[d][
                    self.picking_specs["Metadata"]["Dimensions"]["Width 1"]
                ]
                length2 = data.iloc[d][
                    self.picking_specs["Metadata"]["Dimensions"]["Length 2"]
                ]
                width2 = data.iloc[d][
                    self.picking_specs["Metadata"]["Dimensions"]["Width 2"]
                ]
                terminations = data.iloc[d][
                    self.picking_specs["Metadata"]["Crystal terminations"]
                ]
                geometry = data.iloc[d][
                    self.picking_specs["Metadata"]["Crystal geometry"]
                ]
                ...
                Fts = get_Ft(
                    ...
                    width2,
                    int(terminations),
                    self.picking_specs["geometry_key"][geometry],
                    self.picking_specs["Ft_constants"],
                    material,
                )
                dimensional_mass = (
                    self.picking_specs["Ft_constants"][material]["density"]
                    * Fts["Vcorr"]
                    / 1e6
                )
                ...
                for s in self.picking_specs["Shape"]["attributes"]:
                    ...
                    if "eometry" in col:
                        sparrow_val = self.picking_specs["geometry_key"][
                            int(float(value))
                        ]
                    if "Np" in col:
                        sparrow_val = self.picking_specs["terminations_key"][
                            int(float(value))
                        ]
