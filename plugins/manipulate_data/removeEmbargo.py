...
class RemoveEmbargo(BaseImporter):
    ...
    def remove_embargo(self, file, data_dir):
        ...
        for sample_id in sample_ids['IDs']:
            sample_obj = self.db.session.query(self.db.model.sample).filter_by(lab_id=sample_id).first()
            ...
            if not sample_obj:
                ...
                if sample_obj.member_of:
                    parent_sample = self.db.session.query(self.db.model.sample).filter_by(id=sample_obj.member_of).first()
