...
def split_grain_information(self, df):
    ...
    df[["sample_name", "-", "grain"]] = (
        df["Full Sample Name"].str.replace("_", "-").str.rpartition("-")
    )
    # Go back to previous separators
    df["sample_name"] = df.apply(
        lambda row: row["Full Sample Name"][0 : len(row["sample_name"])], axis=1
    )
    df.drop(
        columns=["-"], inplace=True
    )  # Created a separate column full of "-", which is useless
