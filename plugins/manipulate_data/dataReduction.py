    ...
    def get_input_data(self, d):
        ...
        try:
            ...
            He4_datum = self.query_ID(
                sample_obj.lab_id, "4He blank corrected (±2σ)", datum_unit="fmol"
            )
            ...
            Th232_datum = self.query_ID(
                sample_obj.lab_id, "232Th (±2σ)", datum_unit="ng"
            )
            Th232 = float(Th232_datum.value)
            Th232_s = float(Th232_datum.error) / 2
            Sm147_datum = self.query_ID(
                sample_obj.lab_id, "147Sm (±2σ)", datum_unit="ng"
            )
