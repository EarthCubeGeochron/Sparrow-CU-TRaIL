...
def add_column(ws, col, label, thin):
    ...
    ws.cell(row=2, column=col).alignment = openpyxl.styles.Alignment(
        horizontal="center", vertical="center", wrapText=True
    )
    ws.cell(row=2, column=col).font = openpyxl.styles.Font(
        name="Helvetica Neue", bold=True
    )
...
class PublicationTableExporter(BaseImporter):
    ...
    def create_table(self, data_dir):
        ...
        for dict_ in self.table_specs[:-1]:
            ...
            ws.column_dimensions[openpyxl.utils.get_column_letter(n)].width = dict_[
                key
            ]["width"]
            if dict_[key]["error"]:
                n += 1
        ...
        for sample in sample_dict:
            row += 1
            ws.cell(row=row, column=1, value=sample).font = openpyxl.styles.Font(
                underline="single", size="12"
            )
            for aliquot in sample_dict[sample]:
                row += 1
                ws.cell(
                    row=row, column=1, value=aliquot[0]
                ...
                for dict_ in table_specs[:-1]:
                    ...
                    if is_archival and "Date" in key:
                        item = self.query_datum(aliquot[1], key)
                        if not item:
                            item = self.query_datum(
                                aliquot[1], key.replace("Iterated", "Linear (M&D)")
                            )
                    ...
                    try:
                        if "round" in dict_[key]:
                            ...
                            ws.cell(
                                row=row, column=col, value=rounded_num
                            ).font = openpyxl.styles.Font(size="12")
                            ws.cell(
                                row=row, column=col
                        ...
                        if dict_[key]["error"]:
                            ...
                            col += 1
                            rounded_err = (
                                round(item.error, dict_[key]["error_round"])
                                if dict_[key]["error_round"] != 0
                                else int(item.error)
                            )
                            ws.cell(
                                row=row, column=col, value=rounded_err
                            ).font = openpyxl.styles.Font(size="12")
                            ws.cell(
                                row=row, column=col
                            ).alignment = openpyxl.styles.Alignment(horizontal="center")
                            if "secondary_error" in dict_[key]:
                                col += 1
                                second_err_item = self.query_datum(
                                    aliquot[1], dict_[key]["secondary_error"]
                                )
                                rounded_second_err = (
                                    round(
                                        second_err_item.error,
                                        dict_[key]["secondary_error_round"],
                                    )
                                    if dict_[key]["secondary_error_round"] != 0
                                    else int(second_err_item.error)
                                )
                                ws.cell(
                                    row=row, column=col, value=rounded_second_err
                                ).font = openpyxl.styles.Font(size="12")
                                ws.cell(
                                    row=row, column=col
